...
from utilities.Utility_Functions import normalise_rewards, create_actor_distribution, CategoricalMasked
...
class PPO(Base_Agent):
    ...
    def __init__(self, config):
        ...
        self.policy_new = self.create_NN(input_dim=self.state_size, output_dim=self.policy_output_size, policyGrad=True)
        self.policy_old = self.create_NN(input_dim=self.state_size, output_dim=self.policy_output_size, policyGrad=True)
        ...
        self.experience_generator = Parallel_Experience_Generator(self.environment, self.device, self.policy_new, self.config.seed,
    ...
    def step(self):
        ...
        self.many_episode_states,  self.many_episode_masks, self.many_episode_actions, self.many_episode_rewards = self.experience_generator.play_n_episodes(
    ...
    def calculate_all_ratio_of_policy_probabilities(self):
        ...
        all_masks = [mask for masks in self.many_episode_masks for mask in masks]
        ...
        all_masks = torch.stack([torch.Tensor(masks).float().to(self.device) for masks in all_masks])
        ...
        new_policy_distribution_log_prob = self.calculate_log_probability_of_actions(self.policy_new, all_states, all_masks, all_actions)
        old_policy_distribution_log_prob = self.calculate_log_probability_of_actions(self.policy_old, all_states, all_masks, all_actions)
    ...
    def calculate_log_probability_of_actions(self, policy, states, masks, actions):
        """Calculates the log probability of an action occuring given a policy and starting state"""
        policy_output = policy.forward(states, masks).to(self.device)
        policy_distribution = CategoricalMasked(policy_output, masks, self.device)
