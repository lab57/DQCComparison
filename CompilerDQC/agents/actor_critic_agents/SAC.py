...
class SAC(Base_Agent):
    ...
    def actor_pick_action(self, state=None, eval=False):
        ...
        if eval == False: action, _, _ = self.produce_action_and_action_info(state, mask)
        else:
            with torch.no_grad():
                _, z, action = self.produce_action_and_action_info(state, mask)
        action = action.detach().cpu().numpy()
        return action[0]

    def produce_action_and_action_info(self, state, mask):
        """Given the state, produces an action, the log probability of the action, and the tanh of the mean action"""
        actor_output = self.actor_local(state, mask)
