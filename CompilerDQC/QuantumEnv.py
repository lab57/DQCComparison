...
from QuatumEnvironment import (
    QuantumEvnironment,
)  # import the distributed quantum computing simulation environment
...
class EnvUpdater(
    gymnasium.Env
):  # gym is an opanAI's environment generator tools. however, we doing most of it ourselves for the distributed qunatum computing game
    ...
    def __init__(self, completion_deadline):
        ...
        self.state = (
            self.quantumEnv.state
        )  # state at the beginning decided on by the processor and DAG configurations
        self.mask = (
            self.quantumEnv.mask
        )  # mask at the beginning decided on by the processor and DAG configurations, value always 1 if no masking used

        self.action_dim = (
            self.quantumEnv.generate_action_size()
        )  # environment provides action space size
    ...
    def step(
        self, action
    ):  # this is the main function where each step of the game takes place, i.e. synchroniztion step
        ...
        if (
            action == 0
        ):  ### action=0 is always a stop, and that is the only increase in step
            self.stepCount += 1
        ...
        return (
            ...
            False,
            {},
        )  # supposed to return new state, reward and done to the learning agent
